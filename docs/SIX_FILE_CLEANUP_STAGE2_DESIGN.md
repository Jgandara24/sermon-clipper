...
| `source_media_gates` | source ID PK; state; generation; cleanup operation ID; timestamps | One row per source. Parent deletion removes only an unused generation-0 ACTIVE gate; the trigger protects all gate history. States ACTIVE, QUIESCING, RETIRED, RESTORING. Non-ACTIVE changes require a journaled transition. No cascade deletion of recovery state. |
...
Use RESTRICT rather than CASCADE for recovery-bearing links. The unused gate backfill is the sole exception: a gate with generation 0 and no session or audit history can follow parent deletion. Direct gate deletion is refused. Actor ID snapshots survive account deletion and role revocation. An optional live actor link may use SET NULL. Document workspace erasure handling separately; do not silently erase journals.
...
The current source-copy path now uses project-before-source locks, matching the transcription writers. A concurrent database regression proves this copy/writer case. Do not add a source-first cleanup transaction beside these paths. Gate integration still needs a complete lock-order review and opposite-order race tests before activation. Gate acquisition from a transaction-client helper must require an already-established context; it must not silently nest transactions.
