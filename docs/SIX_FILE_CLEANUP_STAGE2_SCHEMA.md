...
Scope fields cannot change. State changes require the next revision and a matching audit event in the same transaction. Audit history cannot be updated, deleted, or truncated. Parent links use RESTRICT. Actor IDs are snapshots, not live account foreign keys.
...
A partial unique index allows only one HELD reservation for an exact storage identity and key. The reservation has either an operation owner or a session owner. Owner replacement is disabled. A timeout cannot transfer ownership.
...
7. Review source/workspace deletion compatibility. ACTIVE gate foreign keys can block deletion under old code. Define journal retention and account erasure. Do not deploy this migration until this effect is accepted and tested.
