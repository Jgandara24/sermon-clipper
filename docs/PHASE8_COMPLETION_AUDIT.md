...
| --- | --- | --- | --- |
| Real authentication with email OTP and/or Google OAuth | Email OTP uses hashed challenges, DB-backed sessions, Resend delivery, rate limiting, and production readiness failures for missing email config. Google OAuth is intentionally absent. | `npm run verify`; live OTP request and code verification with `RESEND_API_KEY` configured. | Built locally; live delivery proof required. |
...
| Real approval notifications | Approval requests can send email through Resend and SMS through Twilio, with attempts persisted. | Send approval to a real email/SMS recipient in production and confirm notification attempt records. | Built locally; live provider proof required. |
...
| Reliable workers outside the web process | `worker:prod` runs processing and export queues; jobs have attempts, delayed retries, idle process heartbeats, stale-job recovery, and terminal failure states. | `npm run test:integration`; run a separate production worker, confirm `/api/health` reports `worker_heartbeat=ok`, and confirm upload/export events. | Built locally; live worker proof required. |
...
| Health check | Output from `curl -fsS <url>/api/health` showing readiness status `ok`, all checks `ok`, `worker_heartbeat=ok`, and the deployed commit SHA; the evidence text must name `/api/health`, status `ok`, checks `ok`, and commit SHA. |
...
| Worker process | At least one separate worker process is running with stable `WORKER_ID`, available `ffmpeg`/`ffprobe`, configured `ELEVENLABS_API_KEY`, and a recent `worker_heartbeat` health check; the evidence text must name each prerequisite. |
...
| Auth email | A real user receives and verifies an email OTP through Resend; the evidence text must name email OTP, Resend, and verification/sign-in. |
...
| Transcription provider | Operations metadata proves production transcription used base ElevenLabs Scribe v2 with no keyterms. |
...
| Approval notification | A real approval email and/or SMS is delivered; the evidence text must name the approval email/SMS, delivered/sent state, and Resend or Twilio. |
